...
if __name__ == "__main__":
    (X_train, y_train), (X_test, y_test) = utils_thesis4.load_nsl_kdd()

    # Split train set into 10 partitions and randomly use one for training.
    partition_id = np.random.choice(10)
    (X_train, y_train) = utils_thesis4.partition(X_train, y_train, 10)[partition_id]
    ...
    utils_thesis4.set_initial_params(model)

    # Define Flower client
    class MnistClient(fl.client.NumPyClient):
        def get_parameters(self):  # type: ignore
            return utils_thesis4.get_model_parameters(model)

        def fit(self, parameters, config):  # type: ignore
            utils_thesis4.set_model_params(model, parameters)
            # Ignore convergence failure due to low local epochs
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                model.fit(X_train, y_train)
            #print(f"Training finished for round {config['rnd']}")
            return utils_thesis4.get_model_parameters(model), len(X_train), {}
